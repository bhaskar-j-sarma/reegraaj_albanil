<?php
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'add_blogs'){
		...
		$check_duplicate = "SELECT project_name FROM projects WHERE project_name = '".$_REQUEST['project_name']."' ";
		$check_result = $con->data_select($check_duplicate);

		if($check_result == 'no'){

			$product_name = $_REQUEST['project_name'];
			...
			$title = str_replace("'","\'", $_REQUEST['project_name']);
			$category_name = $_REQUEST['project_category'];
			$brand_name = $_REQUEST['project_quote'];
			$description = $_REQUEST['description'];	
			$url = seo_url($title);
			

	    	$sql_insert_blogs = "INSERT INTO projects(project_name, project_category, project_quote, url, description, tbumb_image) VALUES ('".$product_name."','".$category_name."','".$brand_name."','".$url."','".addslashes($description)."','".$blogs_main_image."')";

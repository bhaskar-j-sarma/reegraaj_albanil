...
function shorten_string($string, $wordsreturned){
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'get_all_blogs'){
		...
		$get_all_blogs = "SELECT * FROM projects ORDER BY id DESC";
		...
		if($result_get_all_blogs !='no'){
			...
			foreach ($result_get_all_blogs as $key => $value) {
				  ...
				  $tr .= "<tr>";
                        ...
                        $tr .= "<td>".$result_get_all_blogs[$key]['project_name']."</td>";
                        $tr .= "<td>".$result_get_all_blogs[$key]['project_category']."</td>";
                        $tr .= "<td style=''><div class='btn-group'>".$approvel."</div></td>";
				$tr .= '<td style="width:40%;"><div class="btn-group"><button type="button" data-id="'.$result_get_all_blogs[$key]['id'].'" class="btn btn-success btn-xs view_manage_blogs">View</button>
				<button type="button" data-id="'.$result_get_all_blogs[$key]['id'].'" class="btn btn-danger btn-xs delete_blogs">Delete</button>
				<button type="button" data-id="'.$result_get_all_blogs[$key]['id'].'" class="btn btn-primary btn-xs add_image">Image</button>
				<button type="button" data-id="'.$result_get_all_blogs[$key]['id'].'" class="btn btn-info btn-xs add_location">Location</button>
				<button type="button" data-id="'.$result_get_all_blogs[$key]['id'].'" class="btn btn-primary btn-xs add_features">Features</button>
				<button type="button" data-id="'.$result_get_all_blogs[$key]['id'].'" class="btn btn-danger btn-xs add_ammenities">Ammenities</button>
				<button type="button" data-id="'.$result_get_all_blogs[$key]['id'].'" class="btn btn-success btn-xs add_architects">Architects</button>
				</div></div></td>';
				$tr .= '</tr>';
				$sr++;
			}
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'edit_manage_blogs'){
		...
		$project_name = str_replace("'","\'", $_REQUEST['project_name']);
		$project_category = $_REQUEST['project_category'];
		$project_quote = $_REQUEST['project_quote'];
		$description = $_REQUEST['description'];
		$url = seo_url($project_name);
			
		$sql_update_blogs = "UPDATE projects SET project_name='".$project_name."', project_category='".$project_category."', project_quote='".$project_quote."',description='".addslashes($description)."', tbumb_image = '".$blogs_main_image."',url = '".$url."' WHERE id=".$_REQUEST['id'];
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'delete_blogs'){
		...
		$sql_delete = "DELETE FROM projects WHERE id = ".$_REQUEST['id'];
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'blogs_status'){
		...
		{
			$sql_status = "UPDATE projects SET approvel = 0 WHERE id = ".$_REQUEST['id'];
		}
		else
		{
			$sql_status = "UPDATE projects SET approvel = 1 WHERE id = ".$_REQUEST['id'];
		}
